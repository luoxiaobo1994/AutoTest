...
class Ggr_Debug():
    ...
    def check_view(self):
        while True:
            try:
                print(f"content:{get_time()} {self.driver.app_elements_content_desc(self.view)}")
            except:
                pass
    ...
    def check_image(self):
        while True:
            try:
                print(f"text:{get_time()} {self.driver.app_elements_text(self.view)}")
            except:
                pass

if __name__ == '__main__':
    gg = Ggr_Debug()
    t1 = threading.Thread(target=gg.check_image)
    t2 = threading.Thread(target=gg.check_view)
